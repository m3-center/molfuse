...
def main_report_generation():
    ...
    def create_comparison_barchart(df, metric, y_label, title, filename, latex_content_list, report_figures_dir, log_scale=False, baseline_metric=None):
        ...
        try:
            ...
            if baseline_metric and baseline_metric in df.columns:
                ...
                if pd.notna(mean_baseline):
                    plt.axhline(y=mean_baseline, color='grey', linestyle='--', linewidth=1.5, label=f'Random Baseline (Mean: {mean_baseline:.4f})')
    ...
    if sim_space_only_targets:
        latex_content.append("The following targets were processed in 'similarity_space_only' mode due to limited known ligand data. For these, ranked lists of ZINC compounds were generated for subsequent docking.\n\\begin{itemize}")
