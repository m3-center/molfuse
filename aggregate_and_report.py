...
def get_section_header_latex_standalone(level, title_text):
    sec_cmd_map = {1: r"\section", 2: r"\subsection",
                   3: r"\subsubsection", 4: r"\paragraph", 5: r"\subparagraph"}
...
def add_dataframe_as_latex_table_standalone(latex_content_list, dataframe, caption_text, label_text, placement="[H]", col_format=None, font_size=r"\small"):
    ...
    if dataframe is not None and not dataframe.empty:
        latex_content_list.extend(
            [f"\\begin{{table}}{placement}", r"  \centering"])
        if font_size:
            latex_content_list.append(font_size)
        ...
        for col in df_for_latex.columns:
            if pd.api.types.is_numeric_dtype(df_for_latex[col]):
                ...
                df_for_latex[col] = df_for_latex[col].astype(str).apply(lambda x: "N/A" if pd.isna(
                    x) or (isinstance(x, str) and x.lower() == 'nan') else escape_latex_text_content(x))

        df_for_latex.columns = [escape_latex_text_content(
            str(c).replace('_', ' ').title()) for c in dataframe.columns]
        ...
        latex_content_list.append(
            f"% Table '{escape_latex_text_content(label_text)}' is empty or None.\n")
# --- End LaTeX Helpers ---
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(
        agg_log_file_name, mode='w'), logging.StreamHandler()]
)
    ...
    for rep_info in replicate_run_details:
        ...
        for target_info in config_main['targets']:
            if target_info.get("processing_mode", "full_analysis") == "similarity_space_only":
                logging.debug(
                    f"  Skipping metrics for target {target_info['id_name']} (similarity_space_only).")
                continue
            ...
            for dr_key, dr_params in config_main["dimensionality_reduction_methods"].items():
                ...
                strategies_to_check = [{"dir_leaf": dr_short_name_base.replace(
                    '-', '_'), "label": "Projection" if not dr_key == "tsne" else "Co-embedding (Native)"}]
                if gs.get("run_coembedding_for_pca_umap") and dr_params.get("allow_coembedding") and not dr_key == "tsne":
                    strategies_to_check.append(
                        {"dir_leaf": f"{dr_short_name_base}-Coembed".replace('-', '_'), "label": "Co-embedding"})
                            ...
                            try:
                                ...
                                if not df_m.empty:
                                    ...
                                    for col in df_m.columns:
                                        metric_row[col.replace('%', 'Perc')] = df_m[col].iloc[0] if pd.notna(
                                            df_m[col].iloc[0]) else np.nan
                                    all_metrics_data.append(metric_row)
                            except Exception as e:
                                logging.error(
                                    f"Error reading {metrics_file_path}: {e}")

    if not all_metrics_data:
        logging.error("No metric data collected from any replicates.")
...
def main_report_generation():
    ...
    df_agg.to_csv(os.path.join(report_output_abs_dir,
                  "DEBUG_all_aggregated_metrics.csv"), index=False)

    # --- Overall Performance Summary ---
    latex_content.append(get_section_header_latex_standalone(
        1, 'Overall Comparative Summary of Ranking Performance'))

    # Figure 1: Main Comparison Bar Chart
    logging.info(
        "Generating Figure 1: Main Comparison of Methods by Mean ROC-AUC...")
    try:
        df_fig1_data = df_agg.groupby(['Representation', 'DR_Method', 'Embedding_Strategy'])[
            'ROC_AUC'].agg(['mean', 'std']).reset_index()
        df_fig1_pivot = df_fig1_data.pivot_table(
            index=['Representation', 'DR_Method'], columns='Embedding_Strategy', values=['mean', 'std'])
        df_fig1_pivot.columns = ['_'.join(col).strip()
                                 for col in df_fig1_pivot.columns.values]
        ...
        for col in ['mean_Projection', 'std_Projection', 'mean_Co-embedding', 'std_Co-embedding', 'mean_Co-embedding (Native)', 'std_Co-embedding (Native)']:
            if col not in df_fig1_pivot.columns:
                df_fig1_pivot[col] = 0
        ...
        index = np.arange(len(df_fig1_pivot))
        bar_width = 0.25
        plt.bar(index - bar_width, df_fig1_pivot['mean_Projection'], bar_width,
                yerr=df_fig1_pivot['std_Projection'], label='Projection', color='deepskyblue', capsize=4)
        plt.bar(index, df_fig1_pivot['mean_Co-embedding'], bar_width,
                yerr=df_fig1_pivot['std_Co-embedding'], label='Co-embedding', color='salmon', capsize=4)
        plt.bar(index + bar_width, df_fig1_pivot['mean_Co-embedding (Native)'], bar_width,
                yerr=df_fig1_pivot['std_Co-embedding (Native)'], label='Co-embedding (tSNE Native)', color='mediumseagreen', capsize=4)
        plt.xlabel("DR Method (Representation)")
        plt.ylabel("Mean ROC-AUC (across Replicates, Targets, DIMs)")
        plt.title("Overall Comparison of Methods and Strategies by Mean ROC-AUC")
        plt.xticks(index, df_fig1_pivot['Plot_Label'], rotation=45, ha="right")
        plt.ylim(bottom=0)
        plt.legend(title="Embedding Strategy")
        plt.grid(True, linestyle='--', axis='y')
        plt.tight_layout()

        fig1_path = os.path.join(
            report_figures_abs_dir, "fig1_overall_comparison.png")
        ...
        logging.error(f"Failed to generate Figure 1: {e}", exc_info=True)
        latex_content.append("Figure 1 could not be generated.\n")
    ...
    try:
        ...
        df_table1_agg.fillna(
            {'SD_ROC_AUC': 0, 'SD_PR_AUC': 0, 'SD_Spearman_Rho': 0}, inplace=True)
        df_table1_agg['ROC_AUC'] = df_table1_agg.apply(
            lambda r: f"{r['Mean_ROC_AUC']:.3f} $\\pm$ {r['SD_ROC_AUC']:.3f}" if pd.notna(r['Mean_ROC_AUC']) else "N/A", axis=1)
        df_table1_agg['PR_AUC'] = df_table1_agg.apply(
            lambda r: f"{r['Mean_PR_AUC']:.3f} $\\pm$ {r['SD_PR_AUC']:.3f}" if pd.notna(r['Mean_PR_AUC']) else "N/A", axis=1)
        df_table1_agg['Spearman_Rho'] = df_table1_agg.apply(
            lambda r: f"{r['Mean_Spearman_Rho']:.3f} $\\pm$ {r['SD_Spearman_Rho']:.3f}" if pd.notna(r['Mean_Spearman_Rho']) else "N/A", axis=1)

        df_table1_display = df_table1_agg[[
            'Representation', 'DR_Method', 'Embedding_Strategy', 'ROC_AUC', 'PR_AUC', 'Spearman_Rho']].copy()
        df_table1_display.sort_values(
            by=['Representation', 'DR_Method', 'Embedding_Strategy'], inplace=True)
        ...
        logging.error(f"Failed to generate Table 1: {e}", exc_info=True)
        latex_content.append("Table 1 could not be generated.\n")
    ...
    try:
        mean_roc_per_config = df_agg.groupby(['Target', 'Representation', 'DR_Method', 'Embedding_Strategy', 'DIM'])[
        ...
        df_best_per_target_std = pd.merge(
            df_best_per_target,
            df_agg.groupby(['Target', 'Representation', 'DR_Method', 'Embedding_Strategy', 'DIM'])[
                'ROC_AUC'].std().reset_index().rename(columns={'ROC_AUC': 'SD_ROC_AUC'}),
            on=['Target', 'Representation', 'DR_Method', 'Embedding_Strategy', 'DIM'], how='left'
        ).fillna(0)
        df_best_per_target_std['Mean_ROC_AUC'] = df_best_per_target_std.apply(
            lambda r: f"{r['ROC_AUC']:.3f} $\\pm$ {r['SD_ROC_AUC']:.3f}", axis=1)
        df_best_per_target_display = df_best_per_target_std[[
            'Target', 'Representation', 'DR_Method', 'Embedding_Strategy', 'DIM', 'Mean_ROC_AUC']].copy()
        df_best_per_target_display.rename(
    ...
    try:
        heatmap_data = df_agg.groupby(['Target', 'Representation', 'DR_Method', 'Embedding_Strategy'])[
            'ROC_AUC'].mean().reset_index()
    ...
    logging.info(
        f"Using replicate run '{os.path.basename(representative_replicate_info['path'])}' for illustrative 2D plots.")
    ...
    sim_space_only_targets = [t for t in config['targets'] if t.get(
        "processing_mode") == "similarity_space_only"]
    if sim_space_only_targets:
        latex_content.append(
            "The following targets were processed in 'similarity_space_only' mode due to limited known ligand data in ChEMBL. For these targets, similarity spaces and ranked lists of ZINC compounds were generated for subsequent docking studies, but comparative performance metrics are not applicable.\n\\begin{itemize}")
        for target_info in sim_space_only_targets:
            latex_content.append(
                f"  \\item {escape_latex_text_content(target_info['display_name'])}\n")
        latex_content.append("\\end{itemize}\n")
    else:
        latex_content.append(
            "All targets were processed in 'full_analysis' mode.\n")
    ...
    with open(report_tex_path, "w", encoding='utf-8') as f:
        f.write("\n".join(latex_content))
    logging.info(
        f"Aggregated LaTeX report structure generated: {report_tex_path}")

    try:
        logging.info(
            f"Attempting to compile LaTeX report in: {report_output_abs_dir}")
        for i in range(2):
            process = subprocess.run(
                ["pdflatex", "-interaction=nonstopmode", "-output-directory",
                    report_output_abs_dir, report_tex_path],
                capture_output=True, text=True, check=False)
            if process.returncode != 0:
                logging.error(f"pdflatex compilation pass {i+1} FAILED.")
                latex_log_file = os.path.join(report_output_abs_dir, os.path.basename(
                    report_tex_path).replace('.tex', '.log'))
                if os.path.exists(latex_log_file):
                    ...
                    logging.error(
                        "LaTeX .log file not found. STDERR:\n" + process.stderr)
            else:
                logging.info(f"pdflatex compilation pass {i+1} successful.")
        pdf_path = os.path.join(report_output_abs_dir, os.path.basename(
            report_tex_path).replace('.tex', '.pdf'))
        if os.path.exists(pdf_path):
            logging.info(f"PDF report successfully generated: {pdf_path}")
        else:
            logging.warning(
                f"PDF report {pdf_path} not found after compilation attempts.")
    except Exception as e:
