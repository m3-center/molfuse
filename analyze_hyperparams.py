...
def main_report_generation():
    ...
    latex_content.append("This section synthesizes the results from the hyperparameter sweep to compare the peak performance of each method.")
    
    if all_best_configs:
        df_best_summary = pd.DataFrame(all_best_configs).sort_values(by='ROC_AUC', ascending=False)
        
        # --- New Bar Chart for Peak Performance ---
        plt.figure(figsize=(12, 7))
        sns.barplot(x='ROC_AUC', y='Method', data=df_best_summary, palette='viridis')
        ...
        add_figure_to_latex_standalone(latex_content, os.path.join("figures", fig_filename_summary), caption_summary, "fig_peak_summary")
        ...
        add_dataframe_as_latex_table_standalone(latex_content, df_best_summary, caption_table, "table_best_hyperparams_summary", font_size=r"\scriptsize")
    else:
        latex_content.append("Could not determine best hyperparameter summary.")
    # --- END NEW ---
