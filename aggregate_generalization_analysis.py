...
def main():
    ...
    # Collect metrics from generalization cutoff analysis (includes all 3 target proteins)
    ...
    if args.generalization_cutoff_workspace and os.path.exists(args.generalization_cutoff_workspace):
        logging.info("\nCollecting cutoff analysis metrics for all target proteins...")
    ...
    # If no cutoff data, fall back to collecting from separate workspaces
    if df_gen_cutoff.empty:
        logging.info("\nNo cutoff data found. Collecting from separate workspaces...")
        
        # Collect metrics from ABL1 (use optimal hyperparameters only to match generalization)
        logging.info("Collecting ABL1 metrics (hyperparameter sweep)...")
        ...
        logging.info("Collecting generalization metrics...")
        ...
        # Use only the cutoff data which includes all targets
        df_all = df_gen_cutoff
    ...
    if not df_gen_cutoff.empty and 'Affinity_Cutoff' in df_gen_cutoff.columns:
        logging.info("\n" + "=" * 80)
        ...
        logging.info("\nNo cutoff data available or no cutoff variation found. Using all data for comparison plots.")
    ...
    if not df_gen_cutoff.empty and 'Affinity_Cutoff' in df_gen_cutoff.columns:
        ...
        unique_cutoffs = sorted(df_gen_cutoff['Affinity_Cutoff'].unique())
