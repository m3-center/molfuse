...
def setup_script_logging(log_filename):
    ...
    stream_handler.setFormatter(log_formatter); stream_handler.setLevel(logging.INFO)
...
def save_model(model, path):
    try:
        with open(path, "wb") as f: dump(model, f)
...
def construct_coembedded_dataframe(df_main_info, df_target_info, coembed_coords, dr_col_names):
    ...
    target_info = df_target_info[['SMILES', 'Activity Type', 'Standard Value (nM)', 'accession']].copy()
...
def _parse_fingerprint_chunk(df_chunk):
    """
    Safely parses a DataFrame chunk containing a fingerprint string column.
    Handles non-string and invalid entries gracefully.
    """
    ...
    for idx, s in enumerate(fp_strings):
        if isinstance(s, str):
            try:
                ...
                # This will catch errors if mapping to int fails
                continue # Row remains as NaNs
                
    fp_cols = [f"{FINGERPRINT_COLUMN_PREFIX}{i}" for i in range(NUM_FINGERPRINT_BITS)]
...
def load_and_prepare_data(chembl_mf_path, zinc_path, repr_type, features_list):
    ...
    descriptor_cols = get_descriptor_columns(sample_df, repr_type, features_list)
    if not descriptor_cols:
        raise ValueError("Could not determine descriptor columns from data sample.")
    ...
    for path, source in input_files.items():
        ...
        for chunk in pd.read_csv(path, chunksize=chunksize, low_memory=False):
            ...
            if not chunk.empty:
                valid_info_chunks.append(chunk[[c for c in info_cols_to_keep if c in chunk.columns]].copy())
                valid_descriptor_chunks.append(chunk[descriptor_cols].values.astype(dtype_to_use))
            del chunk; gc.collect()

    if not valid_descriptor_chunks: raise ValueError("No valid data rows found.")
    X_original = np.vstack(valid_descriptor_chunks)
    df_info = pd.concat(valid_info_chunks, ignore_index=True)
    logger.info(f"Loaded {len(df_info)} total valid records. Data matrix shape: {X_original.shape}")
    if 'Compound ChEMBL ID' in df_info.columns and 'ZINC_ID' in df_info.columns:
        df_info['MOLECULE ID'] = df_info['Compound ChEMBL ID'].fillna(df_info['ZINC_ID'])
    else:
        df_info['MOLECULE ID'] = df_info.get('Compound ChEMBL ID', df_info.get('ZINC_ID', 'ID_' + df_info.index.astype(str)))
...
def prepare_target_ligands(path, repr_type, features_list, scaler):
    if not path or path.lower() == 'none' or not os.path.exists(path):
        logger.warning(f"Target ligands file not found or not provided. Co-embedding will be skipped.")
        return None, None, None
    logger.info(f"Preparing held-out active ligands from {path}.")
    df_target_raw = pd.read_csv(path, low_memory=False)
    if df_target_raw.empty: return None, None, None
    descriptor_cols = get_descriptor_columns(df_target_raw, repr_type, features_list)
    df_target_raw.dropna(subset=descriptor_cols, inplace=True)
    if df_target_raw.empty: return None, None, None
    dtype_to_use = np.int8 if repr_type == "fingerprints" else np.float32
    X_target_original = df_target_raw[descriptor_cols].values.astype(dtype_to_use)
    X_target_processed = scaler.transform(X_target_original)
    logger.info(f"Prepared {len(df_target_raw)} target ligands for co-embedding.")
    return df_target_raw, X_target_original, X_target_processed

# --- Dimensionality Reduction Functions (Unchanged) ---
def run_pca(X_processed, df_info, X_target_processed, df_target_info, config, out_paths):
    logger.info("--- Running PCA ---")
    dr_cols = [f'PCA-{i+1}' for i in range(config['simspace_dim'])]
    logger.info(f"Fitting PCA projection model on main data ({X_processed.shape})...")
    model = cumlPCA(**config['cuml_params']) if CUML_AVAILABLE else sklearnPCA(**config['sklearn_params'])
    ...
    if config['run_coembedding'] and X_target_processed is not None:
        ...
        coembed_model = cumlPCA(**config['cuml_params']) if CUML_AVAILABLE else sklearnPCA(**config['sklearn_params'])
        coembed_coords = coembed_model.fit_transform(X_coembed)
        df_coembed = construct_coembedded_dataframe(df_info, df_target_info, coembed_coords, dr_cols)
        df_coembed.to_csv(out_paths['coembed_space'], index=False)
        logger.info(f"Saved co-embedded PCA space to {out_paths['coembed_space']}")
    return pd.DataFrame(projection_coords, columns=dr_cols, index=df_info.index)

def run_tsne(X_pre_reduced, df_info, X_target_pre_reduced, df_target_info, config, out_paths):
    ...
    perplexity = min(config['sklearn_params']['perplexity'], X_coembed.shape[0] - 1)
    model = cumlTSNE(**config['cuml_params']) if CUML_AVAILABLE else sklearnTSNE(perplexity=perplexity, **config['sklearn_params'])
    logger.info(f"Fitting t-SNE on co-embedded data ({X_coembed.shape}) with perplexity={perplexity}...")
    coembed_coords = model.fit_transform(X_coembed)
    df_coembed = construct_coembedded_dataframe(df_info, df_target_info, coembed_coords, out_paths['dr_cols'])
    df_coembed.to_csv(out_paths['coembed_space'], index=False)
    logger.info(f"Saved co-embedded t-SNE space to {out_paths['coembed_space']}")
...
def run_umap_for_metric(metric, X_dict, df_info, df_target_info, config, out_paths):
    ...
    if config['repr_type'] == "features":
        ...
        if metric.lower() in ["jaccard", "hamming"]:
            logger.info(f"UMAP ({metric}) on fingerprints: using RAW, UNSCALED, 2048-D data.")
            X_main, X_target = X_dict['original'], X_dict['target_original']
        else:
            logger.info(f"UMAP ({metric}) on fingerprints: using PCA-REDUCED 50-D data.")
    ...
    use_cuml = CUML_AVAILABLE and metric.lower() not in ["jaccard", "hamming"]
    model = cumlUMAP(**config['cuml_params']) if use_cuml else umapUMAP(metric=metric, **config['sklearn_params'])
    ...
    if config['run_coembedding'] and X_target is not None:
        ...
        coembed_model = cumlUMAP(**config['cuml_params']) if use_cuml else umapUMAP(metric=metric, **config['sklearn_params'])
        coembed_coords = coembed_model.fit_transform(X_coembed)
        df_coembed = construct_coembedded_dataframe(df_info, df_target_info, coembed_coords, out_paths['dr_cols'])
        df_coembed.to_csv(out_paths['coembed_space'], index=False)
        logger.info(f"Saved co-embedded UMAP space ({metric}) to {out_paths['coembed_space']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Calculate similarity spaces using various DR methods.")
    ...
    setup_script_logging(os.path.join(args.output_model_dir, f"calculate_simspaces_{base_name}_seed{args.random_state}.log"))
    features_list = json.loads(args.rdkit_features_list_target_str)
    dr_configs = json.loads(args.dr_method_configs_json_str)

    # 1. Load Data
    df_info, X_original, _ = load_and_prepare_data(args.chembl_mf_data_path, args.zinc_data_path, args.representation_type, features_list)
    ...
    if args.representation_type == "features":
        ...
        logger.info("STEP 2: Using PassthroughScaler for fingerprint data. No scaling will be applied.")
        scaler = PassthroughScaler()
        X_processed_for_dr = scaler.fit_transform(X_original)
    save_model(scaler, os.path.join(args.output_model_dir, f"{base_name}_scaler.lzma"))
    ...
    # 4. Pre-reduce Fingerprints
    X_pca_reduced, X_target_pca_reduced = None, None
    if args.representation_type == "fingerprints":
        logger.info("STEP 4: Pre-reducing fingerprints with PCA for manifold learning.")
        n_comps = min(args.tsne_pca_components, X_processed_for_dr.shape[0] - 1, X_processed_for_dr.shape[1])
        pca_pre_model = cumlPCA(n_components=n_comps) if CUML_AVAILABLE else sklearnPCA(n_components=n_comps)
        ...
        logger.info(f"PCA pre-reduction complete. New shape: {X_pca_reduced.shape}")
    ...
    if args.dr_method_pca:
        # (Unchanged)
        pca_config = {'simspace_dim': args.simspace_dim, 'run_coembedding': args.run_coembedding_for_pca_umap, 'cuml_params': {'n_components': args.simspace_dim, 'random_state': args.random_state}, 'sklearn_params': {'n_components': args.simspace_dim, 'random_state': args.random_state}}
        out_paths = {'projection_model': os.path.join(args.output_model_dir, f"{base_name}_PCA_model.lzma"), 'coembed_space': os.path.join(args.output_simspace_dir, f"{base_name}_PCA_similarity_space_COEMBED.csv")}
        pca_coords = run_pca(X_processed_for_dr, df_info, X_target_processed, df_target, pca_config, out_paths)
        df_results = df_results.join(pca_coords)

    if args.dr_method_tsne and args.simspace_dim == 2:
        # (Unchanged)
        tsne_config = {'cuml_params': {'n_components': 2, 'random_state': args.random_state}, 'sklearn_params': {'n_components': 2, 'perplexity': args.tsne_perplexity, 'random_state': args.random_state, 'n_jobs': -1}}
        out_paths = {'dr_cols': [f't-SNE-{i+1}' for i in range(2)], 'coembed_space': os.path.join(args.output_simspace_dir, f"{base_name}_tSNE_similarity_space_COEMBED.csv")}
        ...
        tsne_coords = run_tsne(X_main_tsne, df_info, X_target_tsne, df_target, tsne_config, out_paths)
        df_results = df_results.join(tsne_coords)

    if args.dr_method_umap:
        # (Unchanged)
        X_data_dict = {'original': X_original, 'processed_for_dr': X_processed_for_dr, 'pca_reduced': X_pca_reduced, 'target_original': X_target_original, 'target_processed': X_target_processed, 'target_pca_reduced': X_target_pca_reduced}
        active_metrics = [m for m in ['euclidean', 'cosine', 'manhattan', 'hamming', 'jaccard'] if getattr(args, f"umap_metric_to_run_{m}")]
        for metric in active_metrics:
            ...
            umap_coords = run_umap_for_metric(metric, X_data_dict, df_info, df_target, umap_config, out_paths)
            df_results = df_results.join(umap_coords)

    # 6. Save Final Projection-based Similarity Space & 7. Cleanup
    logger.info("STEP 6: Saving final combined similarity space file.")
    df_results.to_csv(os.path.join(args.output_simspace_dir, f"{base_name}_similarity_space.csv"), index=False)
