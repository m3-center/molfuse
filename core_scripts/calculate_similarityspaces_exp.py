...
def is_gpu_available():
    ...
    try:
        ...
        if cupy.cuda.runtime.getDeviceCount() > 0:
            logger.info("CUDA device found. cuML (GPU) will be used where possible.")
            return True
        else:
            logger.warning("cupy.cuda.runtime.getDeviceCount() returned 0. No GPU devices found.")
            return False
...
def setup_script_logging(log_filename):
    ...
    logger.info(f"--- calculate_similarityspaces_exp.py logging attached to: {log_filename} ---")
...
def load_and_prepare_data(chembl_mf_path, zinc_path, repr_type, features_list):
    ...
    if not descriptor_cols: raise ValueError("Could not determine descriptor columns.")
...
def run_pca(X_processed, df_info, X_target_processed, df_target_info, config, out_paths, use_gpu):
    ...
    if config['run_coembedding'] and X_target_processed is not None:
        ...
        coembed_model = cumlPCA(**config['cuml_params']) if use_gpu else sklearnPCA(**config['sklearn_params'])
...
def run_tsne(X_pre_reduced, df_info, X_target_pre_reduced, df_target_info, config, out_paths, use_gpu):
    ...
    model = cumlTSNE(**config['cuml_params']) if use_gpu else sklearnTSNE(perplexity=perplexity, **config['sklearn_params'])
...
def run_umap_for_metric(metric, X_dict, df_info, df_target_info, config, out_paths, use_gpu):
    ...
    if config['repr_type'] == "features":
        ...
        if metric.lower() in ["jaccard", "hamming"]:
            logger.info(f"UMAP ({metric}) on fingerprints: using RAW, UNPROCESSED, 2048-D data.")
    ...
    model = cumlUMAP(**config['cuml_params']) if use_cuml_for_metric else umapUMAP(metric=metric, **config['sklearn_params'])
    ...
    if config['run_coembedding'] and X_target is not None:
        ...
        coembed_model = cumlUMAP(**config['cuml_params']) if use_cuml_for_metric else umapUMAP(metric=metric, **config['sklearn_params'])
        coembed_coords = coembed_model.fit_transform(X_coembed)
        df_coembed = construct_coembedded_dataframe(df_info, df_target_info, coembed_coords, out_paths['dr_cols'])
        df_coembed.to_csv(out_paths['coembed_space'], index=False)
...
def main():
    ...
    # (argument parsing...)
    parser.add_argument("--log_file_path", required=True, help="Path to the unified log file for appending.")
    ...
    if args.representation_type == "features":
        ...
        X_processed_for_dr = scaler.fit_transform(X_original)
    save_model(scaler, os.path.join(args.output_model_dir, f"{args.target_id_name}_{args.representation_type}_dim{args.simspace_dim}_scaler.lzma"))
    
    # 3. Prepare Target Ligands
    df_target, X_target_original, X_target_processed = prepare_target_ligands(args.target_ligands_unscaled_path_for_tsne_and_coembed, args.representation_type, features_list, scaler)
    ...
    if args.dr_method_pca:
        ...
        pca_run_config = {'simspace_dim': args.simspace_dim, 'run_coembedding': args.run_coembedding_for_pca_umap, 'cuml_params': {'n_components': args.simspace_dim, 'random_state': args.random_state}, 'sklearn_params': {'n_components': args.simspace_dim, 'random_state': args.random_state}}
        ...
        pca_coords = run_pca(X_processed_for_dr, df_info, X_target_processed, df_target, pca_run_config, out_paths, GPU_ENABLED)
        df_results = df_results.join(pca_coords)

    if args.dr_method_tsne and args.simspace_dim == 2:
        tsne_config = dr_configs.get('tsne', {})
        tsne_run_config = {'cuml_params': {'n_components': 2, 'random_state': args.random_state}, 'sklearn_params': {'n_components': 2, 'perplexity': args.tsne_perplexity, 'random_state': args.random_state, 'n_jobs': -1}}
        ...
        tsne_coords = run_tsne(X_main_tsne, df_info, X_target_tsne, df_target, tsne_run_config, out_paths, GPU_ENABLED)
