...
def setup_script_logging(log_filename):
    ...
    logger.info(
        f"--- Script-specific logging configured. Log file: {log_filename} ---")
...
def load_and_prepare_data(chembl_mf_path, zinc_path, repr_type, features_list):
    ...
    if repr_type == "fingerprints":
        fp_matrix = np.array([list(map(int, s.split(',')))
                             for s in sample_df[PRECALCULATED_FP_STRING_COLUMN_NAME]])
        fp_cols = [f"{FINGERPRINT_COLUMN_PREFIX}{i}" for i in range(
            fp_matrix.shape[1])]
        sample_df = pd.concat([sample_df.drop(columns=[
                              PRECALCULATED_FP_STRING_COLUMN_NAME]), pd.DataFrame(fp_matrix, columns=fp_cols)], axis=1)
    ...
    for path, source in input_files.items():
        ...
        for chunk in pd.read_csv(path, chunksize=chunksize, low_memory=False):
            ...
            if repr_type == "fingerprints":
                fp_matrix = np.array([list(map(int, s.split(',')))
                                     for s in chunk[PRECALCULATED_FP_STRING_COLUMN_NAME]])
                chunk = pd.concat([chunk.drop(columns=[PRECALCULATED_FP_STRING_COLUMN_NAME]), pd.DataFrame(
                    fp_matrix, columns=descriptor_cols, index=chunk.index)], axis=1)
...
def prepare_target_ligands(path, repr_type, features_list, scaler):
    ...
    X_target_original = df_target_raw[descriptor_cols].values.astype(
        dtype_to_use)
    # The scaler will be either a real StandardScaler or the PassthroughScaler
...
def run_pca(X_processed, df_info, X_target_processed, df_target_info, config, out_paths):
    ...
    model = cumlPCA(
        **config['cuml_params']) if CUML_AVAILABLE else sklearnPCA(**config['sklearn_params'])
...
def run_tsne(X_pre_reduced, df_info, X_target_pre_reduced, df_target_info, config, out_paths):
    ...
    model = cumlTSNE(**config['cuml_params']) if CUML_AVAILABLE else sklearnTSNE(
...
def run_umap_for_metric(metric, X_dict, df_info, df_target_info, config, out_paths):
    ...
    if config['repr_type'] == "features":
        ...
        if metric.lower() in ["jaccard", "hamming"]:
            logger.info(
                f"UMAP ({metric}) on fingerprints: using RAW, UNSCALED, 2048-D data.")
    ...
    model = cumlUMAP(**config['cuml_params']) if use_cuml else umapUMAP(
    ...
    if config['run_coembedding'] and X_target is not None:
        ...
        coembed_model = cumlUMAP(
            **config['cuml_params']) if use_cuml else umapUMAP(metric=metric, **config['sklearn_params'])
        coembed_coords = coembed_model.fit_transform(X_coembed)
        df_coembed = construct_coembedded_dataframe(
            df_info, df_target_info, coembed_coords, out_paths['dr_cols'])
        df_coembed.to_csv(out_paths['coembed_space'], index=False)
...
def main():
    ...
    setup_script_logging(os.path.join(
        args.output_model_dir, f"calculate_simspaces_{base_name}_seed{args.random_state}.log"))
    ...
    # --- 2. Scale Data (Features) or Prepare Passthrough (Fingerprints) ---
    if args.representation_type == "features":
        ...
        X_processed_for_dr = scaler.fit_transform(
            X_original)  # This just returns X_original

    save_model(scaler, os.path.join(
        args.output_model_dir, f"{base_name}_scaler.lzma"))

    # --- 3. Prepare Target Ligands for Co-embedding ---
    ...
    if args.representation_type == "fingerprints":
        ...
        # PCA is now run on the original, unscaled binary data
        n_comps = min(args.tsne_pca_components,
                      X_processed_for_dr.shape[0] - 1, X_processed_for_dr.shape[1])
        pca_pre_model = cumlPCA(
            n_components=n_comps) if CUML_AVAILABLE else sklearnPCA(n_components=n_comps)
    ...
    # --- 5. Run Selected DR Methods ---
    logger.info("STEP 5: Running selected dimensionality reduction methods.")
    df_results = df_info.copy()

    if args.dr_method_pca:
        pca_config = {'simspace_dim': args.simspace_dim, 'run_coembedding': args.run_coembedding_for_pca_umap,
        ...
        out_paths = {'projection_model': os.path.join(args.output_model_dir, f"{base_name}_PCA_model.lzma"),
                     'coembed_space': os.path.join(args.output_simspace_dir, f"{base_name}_PCA_similarity_space_COEMBED.csv")}
        pca_coords = run_pca(X_processed_for_dr, df_info,
                             X_target_processed, df_target, pca_config, out_paths)
        df_results = df_results.join(pca_coords)

    if args.dr_method_tsne and args.simspace_dim == 2:
        tsne_config = {'cuml_params': {'n_components': 2, 'random_state': args.random_state},
                       'sklearn_params': {'n_components': 2, 'perplexity': args.tsne_perplexity, 'random_state': args.random_state, 'n_jobs': -1}}
        out_paths = {'dr_cols': [f't-SNE-{i+1}' for i in range(2)],
                     'coembed_space': os.path.join(args.output_simspace_dir, f"{base_name}_tSNE_similarity_space_COEMBED.csv")}
        ...
        tsne_coords = run_tsne(
            X_main_tsne, df_info, X_target_tsne, df_target, tsne_config, out_paths)
    ...
    if args.dr_method_umap:
        ...
        for metric in active_metrics:
            ...
            umap_config = {'repr_type': args.representation_type, 'run_coembedding': args.run_coembedding_for_pca_umap,
                           'cuml_params': {'n_components': args.simspace_dim, 'metric': metric, 'random_state': args.random_state, 'n_neighbors': n_neighbors},
                           'sklearn_params': {'n_components': args.simspace_dim, 'random_state': args.random_state, 'n_neighbors': n_neighbors}}
            out_paths = {'dr_cols': [f'UMAP-{metric.capitalize()}-{i+1}' for i in range(args.simspace_dim)],
                         'projection_model': os.path.join(args.output_model_dir, f"{base_name}_{metric}_UMAP_model.lzma"),
                         'coembed_space': os.path.join(args.output_simspace_dir, f"{base_name}_{metric}_UMAP_similarity_space_COEMBED.csv")}
            umap_coords = run_umap_for_metric(
                metric, X_data_dict, df_info, df_target, umap_config, out_paths)
