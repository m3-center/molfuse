...
def main():
    ...
    parser.add_argument("--output_workspace", default="experiment_workspace_cutoff_analysis_features/", help="Directory to save the new cutoff analysis results.")
    ...
    try:
        with open(args.config_path, 'r') as f:
            # Load the main config for global settings like k_for_knn
    ...
    for original_run_dir in original_run_dirs:
        ...
        # --- FIX: Load the specific config for this run ---
        ...
        # Find which strategies were actually run by checking for their output directories
        strategies_to_check = []
        if os.path.exists(os.path.join(original_run_dir, target_id_name, "results", REPR_TYPE_TO_PROCESS, f"dim_{SIMSPACE_DIM_TO_PROCESS}", dr_params['short_name'].replace('-', '_'))):
            strategies_to_check.append("Projection")
        if os.path.exists(os.path.join(original_run_dir, target_id_name, "results", REPR_TYPE_TO_PROCESS, f"dim_{SIMSPACE_DIM_TO_PROCESS}", f"{dr_params['short_name'].replace('-', '_')}_Coembed")):
            strategies_to_check.append("Co-embedding")
        # Add t-SNE's native co-embedding case
        if dr_method_key == 'tsne' and os.path.exists(os.path.join(original_run_dir, target_id_name, "results", REPR_TYPE_TO_PROCESS, f"dim_{SIMSPACE_DIM_TO_PROCESS}", dr_params['short_name'].replace('-', '_'))):
             strategies_to_check.append("Co-embedding (Native)")
        ...
        for strategy in strategies_to_check:
            for cutoff in affinity_cutoffs:
                ...
                if strategy == "Projection":
                    ...
                    # --- FIX: Use .extend() instead of .append() ---
