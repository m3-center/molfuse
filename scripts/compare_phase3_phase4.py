...
def plot_phase3_phase4_overlay_bedroc160(
    df_phase3: pd.DataFrame,
    df_phase4: pd.DataFrame,
    output_dir: Path
) -> None:
    """
    Overlay Phase 3 and Phase 4 BEDROC (α=160) curves.
    """
    print("\nGenerating Phase 3 vs Phase 4 overlay (BEDROC α=160)...")
    ...
    p3_x = np.array(p3_sorted["mf_size_target_numeric"].values, dtype=float)
    p3_y_mean = np.array(p3_sorted["bedroc_160_mean"].values, dtype=float)
    p3_y_sem = np.array(p3_sorted["bedroc_160_sem"].values, dtype=float)
    ...
    # Phase 4: Cross-target scatter
    p4_x = np.array(p4_umap_feat["natural_mf_size"].values, dtype=float)
    p4_y = np.array(p4_umap_feat["bedroc_160_mean"].values, dtype=float)
    p4_labels = p4_umap_feat["target_short"].values
    ...
    ax.set_xscale("log")
    ax.set_xlabel("Natural MF Cloud Size (compounds)", fontweight="bold", fontsize=13)
    ax.set_ylabel("BEDROC (α=160)", fontweight="bold", fontsize=13)
    ax.set_title("Phase 3 vs Phase 4: BEDROC-160 Across MF Sizes", 
                 fontweight="bold", fontsize=15)
    ax.grid(True, alpha=0.3)
    ax.legend(loc="best", fontsize=11, framealpha=0.95)
    ...
    output_path_png = output_dir / "phase3_phase4_overlay_bedroc160.png"
    output_path_pdf = output_dir / "phase3_phase4_overlay_bedroc160.pdf"
    fig.savefig(output_path_png, dpi=300, bbox_inches="tight")
    fig.savefig(output_path_pdf, bbox_inches="tight")
    plt.close(fig)
...
def plot_phase3_phase4_4panel_comparison(
    ...
    """
    5-panel comparison: EF@1%, BEDROC-20, BEDROC-160, ROC-AUC, PR-AUC (Phase 3 vs Phase 4 overlays).
    
    Compact multi-metric manuscript figure (2×3 grid with 6th subplot hidden).
    """
    print("\nGenerating Phase 3 vs Phase 4 5-panel comparison...")
    ...
    metrics = [
        ...
        ("bedroc_160_mean", "bedroc_160_sem", "bedroc_160_mean", "BEDROC (α=160)", "BEDROC (α=160)"),
    ...
    output_path_png = output_dir / "phase3_phase4_5panel_comparison.png"
    output_path_pdf = output_dir / "phase3_phase4_5panel_comparison.pdf"
    fig.savefig(output_path_png, dpi=300, bbox_inches="tight")
    fig.savefig(output_path_pdf, bbox_inches="tight")
    plt.close(fig)
