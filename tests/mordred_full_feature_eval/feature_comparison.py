...
def compute_mordred(
    ...
    if cached_subset.empty:
        # nothing computed
        stats = {"failed_smiles": smiles_list, "feature_nan_counts": {}, "failure_reasons": stats_ext}
    ...
    # Align to input order (left merge on small subset only)
...
def clean_scale_features(X: pd.DataFrame, train_mask: Optional[np.ndarray] = None) -> Tuple[np.ndarray, List[str]]:
    """Drop columns entirely NaN or zero-variance after imputation, then scale.
    
    Memory-efficient: uses float32 instead of float64 and explicit cleanup.
    """
    # Convert to float32 to halve memory usage (sufficient precision for descriptors)
    ...
    col_mask = (np.nan_to_num(np.std(X_imp, axis=0)) > 1e-6)  # Adjusted threshold for float32
    X_imp_full = X_imp_full[:, col_mask]
    kept_cols = [c for c, k in zip(list(X.columns), col_mask) if k]
    
    # 3) Scale (using float32)
    ...
    X_scaled_full = X_scaled_full.astype(np.float32, copy=False)
    
    # Explicit cleanup
    del X_imp, X_imp_full
    if train_mask is not None:
        del X_scaled_train
    gc.collect()
...
def run_eval(args: argparse.Namespace) -> None:
    ...
    # Cleaning + scaling
    print("[Memory] Cleaning and scaling feature sets...")
    
    # Capture column names BEFORE cleaning/deletion for diagnostics
    ...
    # 5) Visualizations (memory-efficient: plot and save one at a time)
    print("[Memory] Generating visualizations...")
    
    def plot_umap(emb: np.ndarray, labels: np.ndarray, title: str, ax: plt.Axes, xlim: Tuple[float, float], ylim: Tuple[float, float]):
        # Use rasterization for large point clouds to reduce memory
        sns.scatterplot(ax=ax, x=emb[:, 0], y=emb[:, 1], hue=labels, s=10, alpha=0.7, 
    ...
    del all_emb  # Free memory immediately
    gc.collect()
    ...
    plt.tight_layout()
    plt.savefig(out_dir / "umap_all.png", dpi=200, bbox_inches='tight')
    plt.close(fig)
    del fig, axes  # Explicit cleanup
    gc.collect()
    ...
    plt.tight_layout()
    plt.savefig(out_dir / "dist_hist_all.png", dpi=200, bbox_inches='tight')
    plt.close(fig2)
    del fig2, axes2  # Explicit cleanup
    gc.collect()
    ...
    def movement_plot(Z_from: np.ndarray, Z_to: np.ndarray, labels_arr: np.ndarray, title: str, path: Path):
        ...
        for grp in ["target", "mf_cloud", "zinc"]:
            ...
            if m.sum() == 0:
                continue
            ax.scatter(Z_from[m, 0], Z_from[m, 1], s=8, alpha=0.6, color=palette[grp], 
                      label=f"{grp} (start)", rasterized=True)
        # Draw movement vectors (thin, semi-transparent)
        for i in range(Z_from.shape[0]):
            ax.plot([Z_from[i, 0], Z_to_aligned[i, 0]], [Z_from[i, 1], Z_to_aligned[i, 1]], 
                   color="black", alpha=0.05, linewidth=0.5, rasterized=True)
        ax.set_title(title)
        ax.set_xlabel("UMAP-1")
        ax.set_ylabel("UMAP-2")
        ax.set_xlim(*xlim2)
        ...
        plt.close(fig)
        del fig, ax, Z_to_aligned  # Explicit cleanup
        gc.collect()
    ...
    # 6) Save feature datasets in the same format as datasets/molecular_function_features_fingerprints/
    print(f"\n{'='*80}")
    print("SAVING FEATURE DATASETS")
    print(f"{'='*80}")
    
    features_dir = out_dir / "extracted_features"
    features_dir.mkdir(parents=True, exist_ok=True)
    
    # Save features matching datasets/ structure expected by phase1.py:
    # - target_{type}_extracted_features.csv - target active molecules
    # - mf_cloud_{type}_extracted_features.csv - MF cloud (all non-target molecules)
    # - zinc_{type}_extracted_features.csv - ZINC acquirable molecules
    def save_features_by_source(features_scaled: np.ndarray, col_names: List[str], 
                                smiles_series: pd.Series, sources: np.ndarray, 
                                feature_type: str):
        """Save features in datasets/ compatible format expected by phase1.py config."""
        # Create DataFrame from scaled features
        features_df = pd.DataFrame(features_scaled, columns=col_names)
        features_df.insert(0, "smiles", smiles_series.values)
        features_df.insert(1, "source", sources)
        
        # Save target (actives)
        target_mask = (sources == "target")
        if target_mask.any():
            target_df = features_df[target_mask].drop(columns=["source"]).copy()
            filename = f"target_{feature_type}_extracted_features.csv"
            filepath = features_dir / filename
            target_df.to_csv(filepath, index=False)
            print(f"  ✓ target: {len(target_df):,} molecules → {filename}")
            del target_df
            gc.collect()
        
        # Save mf_cloud (molecular function cloud - all non-target MF molecules)
        mf_mask = (sources == "mf_cloud")
        if mf_mask.any():
            mf_df = features_df[mf_mask].drop(columns=["source"]).copy()
            filename = f"mf_cloud_{feature_type}_extracted_features.csv"
            filepath = features_dir / filename
            mf_df.to_csv(filepath, index=False)
            print(f"  ✓ mf_cloud: {len(mf_df):,} molecules → {filename}")
            del mf_df
            gc.collect()
        
        # Save zinc
        zinc_mask = (sources == "zinc")
        if zinc_mask.any():
            zinc_df = features_df[zinc_mask].drop(columns=["source"]).copy()
            filename = f"zinc_{feature_type}_extracted_features.csv"
            filepath = features_dir / filename
            zinc_df.to_csv(filepath, index=False)
            print(f"  ✓ zinc: {len(zinc_df):,} molecules → {filename}")
            del zinc_df
            gc.collect()
    ...
    # Save current 40-feature subset
    print("\nCurrent 40-feature subset:")
    save_features_by_source(X_curr_sc, kept_curr, use_df["smiles"], use_labels, "current40")
    
    # Save full 2D features
    print("\nFull 2D Mordred features:")
    save_features_by_source(X_2d_sc, kept_2d, use_df["smiles"], use_labels, "full2d")
    
    # Save full 2D+3D features
    print("\nFull 2D+3D Mordred features:")
    save_features_by_source(X_2d3d_sc, kept_2d3d, use_df["smiles"], use_labels, "full2d3d")
    
    print(f"\n  All feature files saved to: {features_dir}")
    print(f"\n  Usage in phase1.py config:")
    print(f"    mf_features_csv: {(features_dir / 'mf_cloud_<feature_type>_extracted_features.csv').as_posix()}")
    print(f"    zinc_features_csv: {(features_dir / 'zinc_<feature_type>_extracted_features.csv').as_posix()}")
    print(f"    actives_features_csv: {(features_dir / 'target_<feature_type>_extracted_features.csv').as_posix()}")
    print(f"  Replace <feature_type> with: current40, full2d, or full2d3d")
